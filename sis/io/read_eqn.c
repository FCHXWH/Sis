...
#define YYTRANSLATE(YYX)                                                \
  (0 <= (YYX) && (YYX) <= YYMAXUTOK ? EQN_yytranslate[YYX] : YYUNDEFTOK)
...
static const EQN_yytype_int16 EQN_yytoknum[] =
{
...
static void
EQN_yy_symbol_value_print (FILE *EQN_yyo, int EQN_yytype, YYSTYPE const * const EQN_yyvaluep)
{
...
# ifdef YYPRINT
  if (EQN_yytype < YYNTOKENS)
    YYPRINT (EQN_yyo, EQN_yytoknum[EQN_yytype], *EQN_yyvaluep);
# endif
...
static void
EQN_yy_symbol_print (FILE *EQN_yyo, int EQN_yytype, YYSTYPE const * const EQN_yyvaluep)
{
  YYFPRINTF (EQN_yyo, "%s %s (",
             EQN_yytype < YYNTOKENS ? "token" : "nterm", EQN_yytname[EQN_yytype]);

  EQN_yy_symbol_value_print (EQN_yyo, EQN_yytype, EQN_yyvaluep);
  YYFPRINTF (EQN_yyo, ")");
}
...
static void
EQN_yy_reduce_print (EQN_yy_state_t *EQN_yyssp, YYSTYPE *EQN_yyvsp, int EQN_yyrule)
{
...
#   define EQN_yystrlen(S) (YY_CAST (YYPTRDIFF_T, strlen (S)))
#  else
/* Return the length of YYSTR.  */
static YYPTRDIFF_T
EQN_yystrlen (const char *EQN_yystr)
{
  YYPTRDIFF_T EQN_yylen;
...
static int
EQN_yysyntax_error (YYPTRDIFF_T *EQN_yymsg_alloc, char **EQN_yymsg,
                EQN_yy_state_t *EQN_yyssp, int EQN_yytoken)
{
  enum { YYERROR_VERBOSE_ARGS_MAXIMUM = 5 };
  /* Internationalized format string. */
  const char *EQN_yyformat = YY_NULLPTR;
  /* Arguments of EQN_yyformat: reported tokens (one for the "unexpected",
     one per "expected"). */
  char const *EQN_yyarg[YYERROR_VERBOSE_ARGS_MAXIMUM];
  /* Actual size of YYARG. */
  int EQN_yycount = 0;
  /* Cumulated lengths of YYARG.  */
  YYPTRDIFF_T EQN_yysize = 0;
  ...
  if (EQN_yytoken != YYEMPTY)
    {
      ...
      YYPTRDIFF_T EQN_yysize0 = EQN_yytnamerr (YY_NULLPTR, EQN_yytname[EQN_yytoken]);
          ...
          for (EQN_yyx = EQN_yyxbegin; EQN_yyx < EQN_yyxend; ++EQN_yyx)
            if (EQN_yycheck[EQN_yyx + EQN_yyn] == EQN_yyx && EQN_yyx != YYTERROR
                && !EQN_yytable_value_is_error (EQN_yytable[EQN_yyx + EQN_yyn]))
              {
                ...
                {
                  YYPTRDIFF_T EQN_yysize1
                    = EQN_yysize + EQN_yytnamerr (YY_NULLPTR, EQN_yytname[EQN_yyx]);
...
# undef YYCASE_
...

  {
    /* Don't count the "%s"s in the final size, but reserve room for
       the terminator.  */
    YYPTRDIFF_T EQN_yysize1 = EQN_yysize + (EQN_yystrlen (EQN_yyformat) - 2 * EQN_yycount) + 1;
    if (EQN_yysize <= EQN_yysize1 && EQN_yysize1 <= YYSTACK_ALLOC_MAXIMUM)
      EQN_yysize = EQN_yysize1;
    else
      return 2;
...
EQN_yyparse (void)
{
    EQN_yy_state_fast_t EQN_yystate;
    ...
    EQN_yy_state_t *EQN_yyss;
    EQN_yy_state_t *EQN_yyssp;
  ...
  YYPTRDIFF_T EQN_yymsg_alloc = sizeof EQN_yymsgbuf;
#endif
                                        ...
                                        EQN_yyssp, EQN_yytoken)
      {
        ...
        if (EQN_yysyntax_error_status == 0)
        ...
        else if (EQN_yysyntax_error_status == 1)
          {
            ...
            EQN_yymsg = YY_CAST (char *, YYSTACK_ALLOC (YY_CAST (YYSIZE_T, EQN_yymsg_alloc)));
            if (!EQN_yymsg)
              {
...
/*-----------------------------------------------------.
| EQN_yyreturn -- parsing is finished, return the result.  |
`-----------------------------------------------------*/
EQN_yyreturn:
  if (EQN_yychar != YYEMPTY)
    {
